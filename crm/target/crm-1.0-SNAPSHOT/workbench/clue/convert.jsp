...
<script type="text/javascript">
	$(function(){
		...
		//为搜索操作模态窗口的搜索框绑定时间，执行搜索并展现市场活动列表的操作
		...
		$("#convertBtn").click(function (){
			...
			请求结束后，最终响应回线索列表页
			根据为客户创建交易的复选框有无选中，判断是否需要创建交易
			*/
			if($("#isCreateTransaction").prop("checked")){
				//alert("需要创建交易")
				//需要传递clueId，还要为后台传递交易表单中的信息。金额，预计成交日期，交易名称，阶段，市场活动源的id
				//window.location.href="workbench/clue/convert.do?&money=&expectedDate=&name=&stage=&activityId="
				//以上传递的方式麻烦，表单扩充有风险，挂载参数可能超出上限
				//可以使用提交交易表单的形式，来发出本次的传统请求，提交表单的参数不用手动去挂载，而且可以使用post方式请求
				$("#tranForm").submit();
			}else {
				//alert("不需要创建交易")
				//只传clueId即可
